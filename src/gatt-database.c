...
static void connect_cb_le(GIOChannel *io, GError *gerr, gpointer user_data)
{
    DBG("Called");
	struct btd_adapter *adapter;
	struct btd_device *device;
...
static void send_notification_to_device(void *data, void *user_data)
{
    ...
    if (bacmp(&device_state->bdaddr, &connected_dst) == 0) {
        device = btd_adapter_get_device(notify->database->adapter,
						&device_state->bdaddr, 2);
    } else {
        device = btd_adapter_get_device(notify->database->adapter,
						&device_state->bdaddr,
...
struct btd_gatt_database *btd_gatt_database_new(struct btd_adapter *adapter)
{
	...
	database->le_io = bt_io_listen(connect_cb_le, NULL, NULL, NULL, &gerr,
